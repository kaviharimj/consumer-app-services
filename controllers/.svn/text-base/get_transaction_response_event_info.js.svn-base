...
module.exports.get_transaction_response_event_info =
 function (req, res) {
    var res_arr,re_arr;
        ...
        con.query(queryString_A, function (err_A, rows_A) {
            ...
            if (rows_A.length > 0) {   
              ...
              con.query(queryString_D, function (err_D, rows_D) { 
                ...
                var queryString_U = "select name,mobile_number,email_id,status from users where user_id = '" +user_id + "'  and user_type='"+user_type+"'   AND company_id ='" + rows_A[0].company_id + "'";
                ...
                con.query(queryString_U, function (err_U, rows_U) { 
                    if (rows_U.length > 0) {
                        if(rows_U[0].status == 1){
                            ...
                            //////=========================== MAIN =================================================/////// 
                         var queryString_RES ="SELECT imei,battery_mode,battery_soc FROM ss_battery_history_event_info  WHERE imei = '"+imei+"'  ORDER BY gps_datetime desc limit 0,1";
