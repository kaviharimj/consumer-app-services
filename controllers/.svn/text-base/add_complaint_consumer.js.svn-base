...
module.exports.uploadMultipleImage=async(req,res)=>{ 
  ...
  if(api_key=='') {
    ...
    validation_status = 0;    
      ...
      res_arr = { status: '0', message: 'Please upload atleast 2 images' };
      res.send(res_arr);
      validation_status = 0;
              ...
              if (rows_U.length > 0) {
                  if(rows_U[0].status == 1){
                     ...
                     if (rows_VD2.length > 0) {
                          var queryString_D= "select auto_increment_complaint_suffix,dealer_prefix,billing_state from dealer where dealer_id='"+rows_VD2[0].dealer_id+"'  "; 
                ...
                //   console.log (queryString_VC);     
                        ...
                        //  to: ''+rows_U[0].dealer_email+'', // List of recipients
                          ...
                          cc:'meyyanathan.r@bncmotors.in,sureshdev@bncmotors.in,arn@bncmotors.in,kavitha.s@bncmotors.in,'+rows_U[0].email_id+'',
                        
                       // console.log(FOR TESTING. TO CHECK MAIL SENDING);
