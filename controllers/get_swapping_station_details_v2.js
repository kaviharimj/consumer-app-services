				...
				con.query(queryString_U, function (err_U, rows_U) {
					if (rows_U.length > 0) {
						if(rows_U[0].status == 1) {
							var queryString_SS = "SELECT id, swapping_station_number, station_name, battery_mode_status, (CASE WHEN battery_mode_status = '1' THEN '1-Swapping Initiated' WHEN battery_mode_status = '4' THEN '1-Collect Your Battery' WHEN battery_mode_status = '6' THEN '1-Transaction Completed' WHEN battery_mode_status = '7' THEN '0-Invalid Battery Kept' WHEN battery_mode_status = '8' THEN '0-Failed To Open Empty Door' WHEN battery_mode_status = '9' THEN '0-Failed To Open Withdrawal Door' WHEN battery_mode_status = '10' THEN '0-Empty door opened. But battery not kept' WHEN battery_mode_status = '20' THEN '0-Withdrawal door opened. But battery not taken out' WHEN battery_mode_status = '21' THEN '0-Withdrawal door was not closed. Battery has taken out' WHEN battery_mode_status = '14' THEN '0-LOW_SOC' WHEN battery_mode_status = '16' THEN '0-Sorry to proceed as batteries are available with low SOC. Collect your battery' WHEN battery_mode_status = '22' THEN '0-All cabins are full' ELSE '' END) AS battery_mode_status_val, battery_id_no_accp FROM swapping_station WHERE 1=1 AND imei = '"+imei+"' limit 0,1";
							con.query(queryString_SS, function (err_SS, rows_SS) {
								if(rows_SS.length > 0) {
										...
										if(rows_U[0].wallet_amount >= '52') {
												res_arr = { status: 1, message: 'Sufficient amount found in wallet to proceed', data: rows_SS};
												res.send(res_arr);
										 }else{
